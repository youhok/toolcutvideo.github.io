...
(async function () {
    try {
        ...
        await exec(`ffepeg -i${input} temp/raw-frames%d.png`);
        ...
        for (let count = 1; count <= frames.length; count++) {
            ...
            let frames = await Jimp.read(`temp/raw-frames/${count}.png`);

            frames = await onFrame(frames, count);


            await frame.writeAsyns(`temp/raw-frames/${count}.png`);
